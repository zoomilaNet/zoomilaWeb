...
function AdvertiseCard({isSell, dataSell,dataLet}:{isSell:boolean,dataSell?: SellAd ,dataLet?:LetAd}) {
    ...
    let showSlider:any;
    if(isSell){
         showSlider=dataSell!.photos!.length==0?<div className="w-full h-full bg-no_image flex justify-center items-center"><img src="/image/Zoomila_Logo.svg" alt="آگهی بدون عکس" width="197px" height="51px"/></div>:<Slider basePath={dataSell!.photoPath as string} photos={dataSell!.photos as string[]} photoSlug={dataSell!.slug as string}/>
    }else{
         showSlider=dataLet!.photos!.length==0?<div className="w-full h-full bg-no_image flex justify-center items-center"><img src="/image/Zoomila_Logo.svg" alt="آگهی بدون عکس" width="197px" height="51px"/></div>:<Slider basePath={dataLet!.photoPath as string} photos={dataLet!.photos as string[]} photoSlug={dataLet!.slug as string}/>
    }
    return <div className="w-full h-auto md:w-4/5 lg:w-2/5 flex flex-col border border-black my-2">
        ...
        <div className="w-full h-auto flex flex-col px-2">
            ...
            <div className=" flex flex-row justify-between items-center text-sm md:text-lg my-2 md:my-4">
                <div className="flex flex-row items-center">
                    <img src={data.agencyLogo==null?"/image/aseman.png":data.agencyLogo} alt="agencylogo" width="76px" height="76px" className="border rounded ml-2"/>
...
// {data.rooms!>0?<div className="text-sm md:text-base">{data.rooms} خوابه</div>:null}